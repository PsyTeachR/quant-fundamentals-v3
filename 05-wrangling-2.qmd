...
Until now, we have used the function `as.factor()` which takes an existing variable and converts it to a factor where possible. There are functions which convert variables to each data type, such as `as.character()`, `as.numeric()`, and `as.Date()`. Each variable in a data frame must be the same data type, so you can use them to convert a whole variable to another data type. If there is an errant data entry and it is not possible to convert it to the desired data type, it will replace it with an NA and give you a warning. 
:::
